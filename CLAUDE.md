...
│   ├── const.py            # 프로젝트 전역 상수(원래 각 모듈에 흩어져 있던 것을 한곳에 모음). .env도 여기서 로드
│   └── network.py          # 이 PC의 LAN 내부 IP 자동 탐지 + 시작 배너/대시보드 주소 안내
...
│   │   ├── scan.py         # ScanAPI — 라이브 프레임을 통째로 Gemini에 보내 읽는 스캔 라우트
│   │   ├── calendar.py     # Calendar — 일정 저장소 (병합: 웹 계열)
│   │   ├── light.py        # Light — 조명 프록시, 중계/직접 호출 자동 선택 (병합: 웹 계열)
│   │   └── relay.py        # Relay — 인터넷 중계(ntfy.sh)로 파이에 조명 명령 전달
│   ├── static/theme.css # 공통 디자인 시스템(색 토큰·글꼴·칩/버튼) — 모든 페이지가 링크
│   └── templates/
│       ├── index.html
│       ├── mobile.html  # /mobile — 좌: 듣는 면 / 우: 카메라
│       └── calendar.html # /calendar — 한 달 달력 전용 화면
├── raspberry-server/    # 파이에서 따로 실행하는 서보(조명) 서버 — HTTP 또는 인터넷 중계로 통신
└── docs/
...
| [fundamental/const.py](fundamental/const.py) | 프로젝트 전역 상수. 클래스 하나가 원래 어느 모듈/클래스가 쓰던 상수 묶음인지에 대응(예: `FaceFollowerConst`, `HardwareActuatorConst`) — AX-18A 컨트롤 테이블(레지스터 주소·바이트 크기, `AX_18A`/`ActuatorControlTable`)도 여기 있다. 각 모듈은 여기서 import해서 자기 클래스 속성으로 다시 건다. **`.env` 로딩도 이 파일 맨 위에서 한다** — 아래 클래스들이 import 시점에 바로 `os.environ.get(...)`을 읽으므로, 그보다 늦게 로드하면(과거 `src/app.py`가 그랬다) `.env` 값이 조용히 무시된다 |
| [fundamental/network.py](fundamental/network.py) | `lan_addresses()`/`server_urls()`/`startup_banner()` — 이 PC가 같은 공유기에서 어떤 주소로 보이는지 찾아 폰이 입력할 URL을 만든다. UDP 소켓을 공인 주소로 `connect()`해 실제 패킷 없이 라우팅 테이블만으로 출발지 IP를 알아내는 트릭(`_primary_ipv4`) |
...
| [src/api/light.py](src/api/light.py) | `Light` — `/api/light`. 폰이 파이(HTTP)를 직접 못 부르므로(혼합 콘텐츠 차단) 서버가 대신 호출. `Relay`가 활성화돼 있으면 인터넷 중계로, 아니면 `PI_URL`로 직접 호출 — 둘 다 결과 형태가 같아 호출부는 경로를 몰라도 된다. 파이가 없어도 앱은 뜨고 요청만 502 |
| [src/api/relay.py](src/api/relay.py) | `Relay` — PC와 파이가 서로 다른 공유기에 있어도(둘 다 NAT 뒤) `ntfy.sh` 같은 공개 pub/sub 중계소로 조명 명령을 전달한다. `DESKY_RELAY_TOPIC` 환경변수가 비어 있으면 비활성(`enabled=False`), 그러면 `Light`가 기존 직접 호출로 넘어간다 |
| [raspberry-server/server.py](raspberry-server/server.py) | 파이에서 **따로** 실행하는 서보 서버(`/api/press`) + `RelayClient`(`DESKY_RELAY_TOPIC` 설정 시 중계소 구독, 둘 다 동시에 켜짐). pigpio→lgpio→모의 모드 자동 선택, GPIO 칩 번호는 실제 점유 성공 여부로 판별 |
| [src/api/scan.py](src/api/scan.py) | `ScanAPI` — `POST /api/scan/parse` 하나뿐. 지금 라이브 프레임을 통째로 `Gemini.parse_document()`에 넘겨 읽는다(문서 검출/선택 없음) |
...
main.py
  └─ src.app.DeskyApp                          # 조립만: 서비스 생성 + 라우트 등록 + 실행
       ...
       ├─ src.api.scan.ScanAPI ── Gemini              # 라이브 프레임 통째로 Gemini에 (DocumentScanner 없음)
...
### 문서 스캔 (`src/api/scan.py`, `/mobile`)

`/mobile`의 "문서 스캔" 버튼(또는 음성 "스캔해줘", 가위 제스처) — 지금 라이브로
들어오고 있는 카메라 프레임을 **통째로** Gemini에게 보내 본문을 읽는다.
`POST /api/scan/parse`(바디 `{"mode": "text"|"summary"}`) 하나뿐이다.
`mode="text"`는 원문 그대로, `mode="summary"`는 TTS로 읽어주기 좋은 3문장
이내 요약(`Gemini.parse_document()`). 결과는 화면의 결과 시트에 표시되고
동시에 TTS로 읽힌다(`mobile.html`의 `scanAndRead()`).

별도의 문서 사각형 검출·원근 보정·선택 단계는 없다 — Gemini가 배경의 잡다한
글자는 무시하고 본문만 읽도록 프롬프트(`GeminiConst.DOC_PARSE_INSTRUCTION`)에서
지시한다. 원래는 `perception/document_scanner.py`의 `DocumentScanner`(Canny
엣지 → 윤곽선 검출 → 사각형 근사 → 원근 보정)로 문서를 찾아 잘라낸 뒤 그
영역만 읽혔지만(3라우트: `preview.jpg`로 실시간 미리보기, `detect`로 스냅샷
고정, `parse`로 선택 영역 파싱), 실사용에서는 이 검출 단계 없이 라이브
프레임을 그냥 보내는 것으로 충분해 걷어냈다 — `DocumentScanner`는 이제
이 프로젝트 어디서도 쓰지 않는다.
...
**조명** (`src/api/light.py`, `src/api/relay.py`, `raspberry-server/`) — 경로가 한 단계
더 있는 이유가 있다:

```text
폰 ──HTTPS──▶ 이 서버 (/api/light) ──HTTP 또는 인터넷 중계──▶ 파이 (/api/press) ──PWM──▶ 서보
```
...
PC → 파이 구간은 설정에 따라 자동으로 둘 중 하나로 갈린다:

- **직접 호출**(기본) — 같은 공유기에 있을 때. `LightConst.PI_URL`로 곧바로 HTTP 호출.
- **인터넷 중계** — PC와 파이가 서로 다른 공유기에 있으면 `PI_URL`로는 서로 닿을 수
  없다(둘 다 NAT 뒤). `DESKY_RELAY_TOPIC` 환경변수를 양쪽에 같은 값으로 넣으면
  `src/api/relay.py`의 `Relay`가 이 값을 켠다 — PC와 파이가 **둘 다 바깥으로 나가서**
  공개 pub/sub 서비스(`ntfy.sh`)에서 만난다. 나가는 연결은 웹 브라우징과 같아 포트
  포워딩·터널·고정 IP가 전혀 필요 없다. 주제 이름이 곧 비밀번호이므로 길고 무작위한
  값을 쓸 것(`python -c "import secrets; print('desky-' + secrets.token_urlsafe(16))"`).
  파이 쪽 짝은 `raspberry-server/server.py`의 `RelayClient` — 로컬 HTTP 서버와 **동시에**
  뜬다. `Light.status()`(`GET /api/light/status`)가 지금 어느 경로를 쓰는지와 파이의
  생존 여부(`pi_alive`, 30초 주기 하트비트 기준)를 알려준다.

**LAN 주소 안내** (`fundamental/network.py`) — 서버는 `0.0.0.0`(모든 랜카드)에
바인딩되므로 같은 공유기의 폰은 이미 접속할 수 있는 상태지만, `0.0.0.0`은 주소창에
입력할 수 없고 `localhost`는 폰 자신을 가리켜 정작 입력해야 할 주소를 사람이 몰랐다.
`network.py`가 UDP 소켓을 공인 주소로 `connect()`해(패킷은 안 나간다 — 커널 라우팅
테이블만으로 출발지 IP를 알아낸다) 이 PC의 실제 공유기 내부 주소를 찾고, 서버 시작
시 콘솔 배너로, 대시보드(`GET /api/network`)에서는 복사 가능한 카드로 보여준다.
...
`FaceFollower(yaw_gain=...)`처럼 인스턴스 생성 시 값을 덮어쓰는 기존 메커니즘도 안 깨진다.
...
- 모든 모듈의 콘솔 출력은 `fundamental.logger.Logger.log(tag, message)`를 거쳐 `[TAG] message` 형식으로
  통일된다 (`CONTROLLER`, `ACTUATOR`, `WEBAPP`, `GEMINI`, `STT`, `SCAN`, `HAND`,
  `CAMERA`, `KINEMATICS`, `URDF` 태그). `Logger.enabled` 플래그 하나로 전체 로그를 끌 수 있다.
- `Gemini`는 하드웨어와 같은 "부분 실패" 패턴을 따른다: 생성자에서
