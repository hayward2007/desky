...
| [src/app.py](src/app.py) | Flask 앱 + `run()` — 위치/관절각 제어 대시보드(`/`), 모바일 카메라 웹소켓(`/ws/camera`, `/mobile`), Gemini 채팅/요약/문서스캔 API, 서버 로컬 cv2 미리보기 창(얼굴/손 인식 오버레이 포함) |
...
main.py
  └─ src.app (Flask) ── hardware.actuator.ArmController ── hardware.actuator.Actuator ×5 ── hardware.controller.Controller ── fundamental.const.AX_18A + dynamixel_sdk
       ...
       └─ perception.follow_controller.FollowController ── perception.face_tracker.{FaceTracker,FaceFollower}
                                                        └─ perception.hand_tracker.{HandTracker,HandFollower}
          (run()의 로컬 cv2 미리보기 루프 전용, 라우트 아님 — FaceTracker만 서버에서
          mediapipe로 인식하고, HandTracker는 휴대폰이 보낸 랜드마크의 좌표 변환만 한다)
```
