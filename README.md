...
휴대폰 카메라를 통해 **얼굴/손 인식**(얼굴은 서버 mediapipe, 손은 휴대폰 브라우저 자체
MediaPipe Tasks Vision), **문서 스캔/요약**, **음성 대화**(Gemini)도 지원한다.
...
- 의존성은 [requirements.txt](requirements.txt)에 정리돼 있다:
  ...
  - `mediapipe` — [perception/face_tracker.py](perception/face_tracker.py) 얼굴 인식(손 인식은
    휴대폰 브라우저가 자체적으로 하므로 서버 의존성이 아님 — 아래 "얼굴/손 인식" 절 참고)
...
│   ├── camera_geometry.py
│   ├── face_tracker.py     # mediapipe FaceMesh, 서버 실행
...
| [perception/face_tracker.py](perception/face_tracker.py) | `FaceTracker` — mediapipe FaceMesh 얼굴 인식(서버 실행) + `FaceFollower` |
...
| [perception/follow_controller.py](perception/follow_controller.py) | `FollowController` — 얼굴>손>idle 우선순위 + idle 두리번 상태 머신 |
...
main.py
  └─ src.app.DeskyApp                       # 조립만: 서비스 생성 + 라우트 등록 + 실행
       ...
       └─ src.perception_loop.PerceptionLoop
            ├─ perception.face_tracker.FaceTracker        (서버 mediapipe)
...
### 얼굴/손 인식 (`perception/face_tracker.py`, `perception/hand_tracker.py`)

**얼굴은 서버, 손은 휴대폰이 인식한다** — 컴퓨터 하나가 얼굴+손 인식을 mediapipe로 둘 다
처리하면 매 프레임 렌더링/하드웨어 제어까지 합쳐서 휴대폰이 실제로 보내는 프레임레이트
(~20fps)를 못 따라갔기 때문에 나눴다. `src.app.run()`의 로컬 미리보기 창이 매 프레임
mediapipe FaceMesh로 얼굴을 인식하고, `mobile.html`이 휴대폰 브라우저에서 직접 MediaPipe
Tasks Vision `HandLandmarker`를 돌려 손 랜드마크만 `/ws/camera`로 보낸다 — 서버는 그 좌표를
3D 월드 좌표로 변환만 한다. 카메라 창에 얼굴/손 골격을 오버레이하고, 3D 씬 창에도 추정 월드
좌표를 함께 그린다. 휴대폰 카메라가 end-effector에 달려 있다는 전제로, 기준자(손목~엄지CMC
3.5cm, 눈 바깥쪽 끝 사이 9cm)와 화면상 크기로 카메라까지 거리를 역산(핀홀 모델)한 뒤 현재
end-effector의 FK 변환행렬로 랜드마크를 월드 좌표에 올린다. 실제 캘리브레이션한 값이 아니므로
거리는 추정치다. mediapipe가 서버에 없으면 얼굴 오버레이만 빠지고, 휴대폰이 HandLandmarker를
못 띄우면 손 오버레이만 빠진다 — 어느 쪽이든 카메라 창은 그대로 뜬다.
...
쓴다 — 그래서 손 인식이 두 번 돌지 않는다. `mobile.html`의
`HAND_TRACKING_ENABLED`를 false로 두면 손 추종과 제스처가 **함께** 꺼지고, 얼굴 추적과
idle 두리번거림, 버튼·음성 기능은 그대로 남는다. 폰 화면의 "제스처 켬/끔" 버튼은
폰 쪽 실행만 막는 스위치다.
