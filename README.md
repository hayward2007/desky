...
휴대폰 카메라를 통해 **얼굴/손 인식**(둘 다 휴대폰 브라우저 자체 MediaPipe Tasks Vision,
얼굴 인식 실패 시 사람 몸 인식으로 서버가 폴백), **문서 스캔/요약**, **음성 대화**(Gemini)도
...
- 의존성은 [requirements.txt](requirements.txt)에 정리돼 있다:
  ...
  - 얼굴/손 인식 자체는 서버 의존성이 아니다 — 휴대폰 브라우저가 MediaPipe Tasks
    Vision을 자체적으로 돌린다(아래 "얼굴/몸/손 인식" 절 참고).
  - `mediapipe`(Python) — [perception/body_tracker.py](perception/body_tracker.py) 몸(어깨)
    인식. 얼굴 인식이 실패했을 때만 서버가 직접 도는 폴백이라, 이 패키지가 필요한 건
    그때뿐이다.
...
| [perception/body_tracker.py](perception/body_tracker.py) | `BodyTracker` — **서버에서 직접 mediapipe Pose로 인식**(얼굴 인식 실패 시 폴백) + `BodyFollower` |
...
| [src/perception_loop.py](src/perception_loop.py) | `PerceptionLoop` — 프레임→얼굴/손→추종 명령→제스처→미리보기. 세 가지 주기를 따로 관리 |
...
| [src/api/camera.py](src/api/camera.py) | `Camera` — `/ws/camera` **양방향**: 폰→서버(프레임·음성·손 랜드마크), 서버→폰(transcript·제스처 명령) |
...
main.py
  └─ src.app.DeskyApp                       # 조립만: 서비스 생성 + 라우트 등록 + 실행
       ...
       └─ src.perception_loop.PerceptionLoop
            ...
            ├─ perception.body_tracker.BodyTracker        (서버 mediapipe Pose, 얼굴 폴백)
...
**얼굴/손 인식은 휴대폰이 한다** — 컴퓨터 하나가 얼굴+손 인식을 mediapipe로 둘 다
...
직접 MediaPipe Tasks Vision(`HandLandmarker` + `FaceLandmarker`)을 돌려 랜드마크만
`/ws/camera`로 보내고, 서버는 그 좌표를 3D 월드 좌표로 변환만 한다. 카메라 창에 얼굴/손
골격을 오버레이하고, 3D 씬 창에도 추정 월드 좌표를 함께 그린다. 휴대폰 카메라가
end-effector에 달려 있다는 전제로, 기준자(손목~엄지CMC 3.5cm, 눈 바깥쪽 끝 사이 9cm)와
화면상 크기로 카메라까지 거리를 역산(핀홀 모델)한 뒤 현재 end-effector의 FK 변환행렬로
랜드마크를 월드 좌표에 올린다. 실제 캘리브레이션한 값이 아니므로 거리는 추정치다. 휴대폰이
HandLandmarker/FaceLandmarker 중 하나를 못 띄우면(오프라인, CDN 접근 불가 등) 그
오버레이만 빠진다 — 서로 독립적으로 실패하며, 어느 쪽이든 카메라 창은 그대로 뜬다. 손
인식/가위바위보 제스처는 켜져 있다(`mobile.html`의 `HAND_TRACKING_ENABLED = true`) —
...
**몸(어깨) 인식만 예외적으로 서버가 한다** — 얼굴 인식이 실패했을 때(옆모습, 고개를 돌린
경우 등) 폴백으로, `mediapipe`(Python)가 이 프로젝트에 남은 유일한 이유다. 양쪽 어깨
중점을 몸 중심으로 쓰고 어깨 너비(0.4m 가정)로 깊이를 추정하는 것 말고는 얼굴/손과 같은
핀홀 방식이고, 추종 로직(화면 오프셋 → yaw 회전 + 높이 이동)도 `FaceFollower`와 동일하다.
...
어깨는 상대적으로 안정적으로 잡히기 때문. 같은 기준으로 얼굴이 가까우면 몸 인식 자체를
건너뛰어 상시 비용을 줄인다.
...
`FACE_DEPTH_SMOOTHING`으로 별도 평활). 몸 인식 실행 여부(`need_body`)도 이 히스테리시스
폭만큼 더 일찍 켜서, 실제 전환 시점에 몸 데이터가 이미 준비돼 있게 한다.
...
2. 메인 스레드에서 `/ws/camera`로 들어온 최신 프레임을 얼굴/손 인식 오버레이와 함께
   `cv2.imshow("Mobile camera", ...)`로, 로봇팔 3D 자세 + 인식된 얼굴/손 위치를 "3D scene
   (robot + hand/face)" 창으로 서버가 실행 중인 컴퓨터 화면에 띄운다(이 미리보기 갱신은
...
- `BodyTracker`/`Gemini`/`DocumentScanner`는 하드웨어와 같은 "부분 실패" 패턴을 따른다 —
  의존성이 없어도 앱 전체가 죽지 않고 그 기능만 비활성화된다. (`FaceTracker`/`HandTracker`는
  더 이상 이 패턴이 아니다 — 인식 자체를 안 하므로 서버 쪽엔 `available` 개념이 없다.)
