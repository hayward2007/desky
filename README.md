...
| [src/api/light.py](src/api/light.py) | `Light` — 조명 스위치 서보 프록시(`/api/light`). 폰의 혼합 콘텐츠 차단을 서버 경유로 우회 |
| [raspberry-server/server.py](raspberry-server/server.py) | 파이에서 **따로** 실행하는 서보 제어 서버 (`/api/press`) — 이 앱과 HTTP로만 통신 |
...
main.py
  └─ src.app.DeskyApp                       # 조립만: 서비스 생성 + 라우트 등록 + 실행
       ...
       ├─ src.api.light.Light ──HTTP──▶ raspberry-server  (웹 기능 계열)
...
```text
폰 ──HTTPS──▶ 이 서버 (/api/light) ──HTTP──▶ raspberry-server (/api/press) ──PWM──▶ 서보
```

파이 쪽은 [raspberry-server/server.py](raspberry-server/server.py)를 파이에서 따로
실행한다(pigpio → lgpio → 모의 모드 자동 선택). 파이가 꺼져 있어도 이 앱은 그대로 뜨고
조명 요청만 502로 실패한다. 주소는 환경변수 `PI_URL`로 바꾼다.
