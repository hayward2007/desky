...
| [src/app.py](src/app.py) | Flask 앱 + `run()` — 위치/관절각 제어 대시보드(`/`), 모바일 카메라 웹소켓(`/ws/camera`, `/mobile`), Gemini/문서스캔 API, 서버 로컬 cv2 미리보기 창(얼굴/손 인식 오버레이 포함) |
...
main.py
  └─ src.app (Flask) ── hardware.actuator.ArmController ── hardware.actuator.Actuator ×5 ── hardware.controller.Controller ── fundamental.const.AX_18A + dynamixel_sdk
       ...
       └─ perception.follow_controller.FollowController ── face_tracker + hand_tracker (로컬 cv2 미리보기 전용)
```
