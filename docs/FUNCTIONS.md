...
각 모듈이 여기서 import해 자기 클래스 속성으로 다시 건다.
...
| 함수 | 설명 |
|------|------|
| `DocumentScanner.detect(frame)` | 축소 → Canny → 사각형 근사 → 중복 제거 순으로 문서를 찾는다 |
| `DocumentScanner.draw(frame, documents, selected_id)` | 검출 결과에 번호를 그려 넣는다 |
| `DocumentScanner.four_point_transform(frame, quad)` | 선택한 문서를 반듯하게 편다(원근 보정) |
| `DocumentScanner.order_points(pts)` | 네 꼭짓점을 좌상→우상→우하→좌하 순서로 정렬 |
...
| `start_server()` | Flask를 백그라운드 스레드로 실행(`app.run()`이 블로킹이므로) |
...
| `scan.ScanAPI.preview/detect/parse` | 스냅샷 고정 → 문서 검출 → 선택 문서 읽기 |
...
| `light.Light.command/status` `[웹]` | `/api/light`, `/api/light/status` |
...
| 함수 | 설명 |
|------|------|
| `Servo._pick_backend()` | pigpio(파이 4 이하) → lgpio(파이 5) → 모의 모드 자동 선택 |
