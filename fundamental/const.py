...
# .env를 **여기서** 읽는다 — 이 파일보다 나중에 읽으면 늦기 때문이다.
#
# 아래 클래스들은 `PI_URL = os.environ.get(...)`처럼 **클래스 본문에서 곧바로**
# 환경변수를 읽는다. 클래스 본문은 이 모듈이 import되는 순간 실행되므로, 그
# 시점에 이미 값이 환경에 들어와 있어야 한다. 예전에는 src/app.py가
# `from fundamental.const import ...` 뒤에 load_dotenv()를 불렀는데, import가
# 먼저 실행되는 바람에 .env에 적은 PI_URL·DESKY_RELAY_TOPIC이 조용히 무시되고
# 기본값이 박혔다(값을 넣었는데 아무 일도 안 일어나는, 알아채기 어려운 종류의
# 문제였다). 모든 진입점이 결국 이 모듈을 import하므로 여기서 읽으면 항상
# 제때 반영된다.
try:
    ...
    # python-dotenv가 없어도 동작한다 — 그때는 진짜 환경변수로 설정해야 한다.
    pass
...
class FollowControllerConst:
    ...
    # FollowController가 생성된 시점(=앱/로봇을 막 켠 시점)부터 이 시간(초)
    # 동안은 얼굴/몸/손이 보여도 전부 무시하고 무조건 idle(IDLE_POSITION
    # 복귀 → 두리번거리기)로 유지한다 — 전원을 켜자마자 카메라 앞에 사람이
    # 있으면 웜업도 안 끝난 상태에서 팔이 곧바로 그쪽으로 홱 움직이는 것을
    # 막기 위함(`FollowController.next_command` 맨 앞에서 처리). 이 시간이
    # 지나면 그 시점에 보이는 대상으로 디바운스 없이 바로 추적을 시작한다
    # (idle에서 뭔가 나타났을 때의 기존 규칙과 동일).
    STARTUP_IDLE_S = 3.0
    ...
    # 지금 얼굴을 추적 중일 때만 적용하는, TARGET_SWITCH_TIMEOUT_S보다 긴
    # 전용 유예 시간(초) — 얼굴이 잠깐 안 보인다고(고개를 돌리거나 눈을
    # 깜빡이는 정도로도 흔함) 바로 몸(어깨)으로 넘어가지 않고 "다시 찾을
    # 시간"을 더 준다. 그 외 전환(몸->얼굴, 손 관련, idle 복귀 등)은 여전히
    # TARGET_SWITCH_TIMEOUT_S를 쓴다 — 얼굴을 잃을 때만 유독 더 참을성 있게
    # 만들고 싶다는 요청으로 추가됐다(`FollowController._update_active_target`).
    FACE_LOST_GRACE_S = 1.5

    # 몸(어깨)을 추적 중일 때, 화면 오프셋이 데드존 안이라 팔이 이 시간(초)
    # 동안 계속 가만히 있었으면(=사람이 멈춰 있었으면) 얼굴을 다시 찾아보려고
    # 팔을 한 번 살짝 위로 들어 올린다 — 몸(어깨) 기준으로 카메라를 맞추면
    # 각도상 얼굴이 화면 위쪽 경계 밖에 걸쳐 있을 수 있는데, 조금만 올려도
    # 다시 잡히는 경우가 많다. FACE_BODY_SWITCH_DISTANCE_M보다 가까울 때만
    # 시도한다("가까울 때" — 멀면 애초에 얼굴을 잡을 가능성이 낮다). 한 번
    # 들어 올린 뒤에는 그 자세를 FACE_REACQUIRE_SUPPRESS_S 동안 유지하며
    # (BodyFollower가 곧바로 다시 아래로 되돌리지 않도록) 얼굴이 잡히길
    # 기다리고, 이 몸 추적 세션 동안은 한 번만 시도한다(계속 실패해도
    # 무한정 위로 계속 들리지 않도록 — `FollowController._reacquire_attempted`).
    FACE_REACQUIRE_STILL_S = 2.0
    # 위 조건이 충족됐을 때 한 번에 들어 올리는 높이(m) — 자동 추종 스텝
    # (BodyFollowerConst.HEIGHT_STEP_LIMIT)보다 뚜렷하게 커야 "한번 훑어보는"
    # 의도된 동작으로 보인다.
    FACE_REACQUIRE_HEIGHT_STEP_M = 0.04
    # 들어 올린 자세를 유지하며 얼굴 재인식을 기다리는 시간(초) — 이 동안은
    # BodyFollower의 보정 명령을 무시한다(안 그러면 몸 기준으로 즉시 다시
    # 낮춰버려서 들어 올린 의미가 없어진다).
    FACE_REACQUIRE_SUPPRESS_S = 1.2
...
class LightConst:
    ...
    # --- 인터넷 중계 (다른 망에 있어도 동작) ------------------------------
    # PC와 파이가 서로 다른 공유기에 있으면 위 PI_URL로는 닿을 수 없다(둘 다
    # NAT 뒤에 있다). 그때는 양쪽이 **바깥으로 나가서** 공용 중계소에서 만난다.
    # 자세한 이유와 보안 주의는 src/api/relay.py 모듈 docstring 참고.
    #
    # 주제 이름이 곧 비밀번호다. 비워 두면 중계를 쓰지 않고 PI_URL 직접 호출만
    # 한다(같은 공유기 전용). 양쪽에 같은 값을 넣어야 하며, 만드는 법:
    #   python -c "import secrets; print('desky-' + secrets.token_urlsafe(16))"
    RELAY_TOPIC = os.environ.get("DESKY_RELAY_TOPIC", "")
    # 중계소 주소. 무료 공개 서비스라 가입도 API 키도 필요 없다.
    RELAY_URL = os.environ.get("DESKY_RELAY", "https://ntfy.sh").rstrip("/")
    # 명령을 보내고 파이의 응답을 기다리는 시간(초). 인터넷을 두 번 왕복하고
    # 서보가 실제로 움직이는 시간까지 포함하므로 LAN 직접 호출보다 넉넉하다.
    RELAY_TIMEOUT_S = 6.0
    # 파이의 마지막 신호가 이 시간 안이면 "살아 있다"고 본다(초).
    # 파이는 30초마다 생존 신호를 보낸다.
    RELAY_ALIVE_S = 90.0
